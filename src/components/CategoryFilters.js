  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <Link className="badge badge-light m-1 p-2 fs-100" to={``} onClick={() => filterBy("All")}>
          All <span className="badge badge-light ml-2">{deals.length}</span>
        </Link>
        {categories.length && categories.map(category => (
          <Link key={category} className="badge badge-primary m-1 p-2 fs-100 small" to={``} onClick={() => filterBy(category)}>
            {category} <span className="badge badge-light rounded ml-2">{this.getNumberOfPosts(category)}</span>
          </Link>
        ))}
      </div>
    );
